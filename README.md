...
*Advanced fleet management optimization for enterprise supply chain operations*

---

## 📋 Overview

**Fleet Sizing Model** addresses a critical challenge in modern supply chain management: fleet management. This implementation combines rigorous academic methodology with production-ready Python code, suitable for both research and enterprise deployment.

Built on the foundational work of **Professor Gilbert Laporte**, this tool provides supply chain professionals with an analytical framework that transforms raw operational data into actionable optimization decisions. Whether you're managing a single warehouse or a global multi-echelon network, this toolkit scales to your complexity.
...
Supply chain fleet management is a persistent operational challenge that impacts cost, service, and working capital across the enterprise. Organizations that fail to optimize fleet management typically see:
...
```bash
git clone https://github.com/virbahu/fleet-sizing-model.git
cd fleet-sizing-model
pip install -r requirements.txt
python fleet_sizing_model.py
```
...
```python
# Quick start example
from fleet_sizing_model import *
...
# See docstrings in fleet_sizing_model.py for full parameter reference
```
...
| | |
|---|---|
| **Based on** | Professor Gilbert Laporte, HEC Montreal |
| **Key Reference** | Laporte (1992) *The Vehicle Routing Problem.* INFORMS |
